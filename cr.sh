...
install_chart_releaser() {
    ...
    if [[ ! -d "$cache_dir" ]]; then
        ...
        curl -sSLo cr.tar.gz "https://github.com/fpm-git/chart-releaser/releases/download/$version/chart-releaser_${version#v}_linux_amd64.tar.gz"
        tar -xzf cr.tar.gz -C "$cache_dir"
        chmod a+x -R "$cache_dir"
        rm -f cr.tar.gz
